...
namespace LAM
{
    ...
    bool ConjugateGradient_MultiGPUS_CUDA_NCCL<FloatingType>::solve(int max_iters, FloatingType rel_error)
    {
        ...
        *  ---------------------------------------------------------------
        */
        // timing the NCCL initialization
        ...
        */
        ...
        ncclCommInitRank(&nccl_comm, num_procs , id, rank);
    
        auto nccl_end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> nccl_elapsed = nccl_end - nccl_start;

        if(rank == 0) {
            std::cout << nccl_elapsed.count() << ",";
        }
        /*
        *  ---------------------------------------------------------------
        *  -----------------  CG Algorithm  ------------------------------
        ...
        */ 
        ...
        for(num_iters = 1; num_iters <= max_iters; num_iters++)
        {
            ...
            ncclBroadcast(p_dev, p_dev, _num_cols, nccl_datatype, 0, nccl_comm, stream);
            ...
            if(rank==0){
                for(int i=0; i<num_procs; i++){
                    ncclRecv(Ap0_dev + _displs[i], _sendcounts[i], nccl_datatype, i, nccl_comm, stream);
                }
            }
            ncclSend(Ap_dev, _num_local_rows, nccl_datatype, 0, nccl_comm, stream);
            ncclGroupEnd();
  ...
  template<typename FloatingType>
    bool ConjugateGradient_MultiGPUS_CUDA_NCCL<FloatingType>::generate_matrix(const size_t num_rows, const size_t num_cols){
        int rank, num_procs, localRank = 0;
        MPI_Comm_rank(MPI_COMM_WORLD, &rank);
        MPI_Comm_size(MPI_COMM_WORLD, &num_procs);
