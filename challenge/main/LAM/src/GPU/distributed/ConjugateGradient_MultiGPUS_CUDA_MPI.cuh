...
namespace LAM
{
    ...
    bool ConjugateGradient_MultiGPUS_CUDA_NCCL<FloatingType>::solve( int max_iters, FloatingType rel_error)
    {
        ...
        if (myRank == 0) {
            ...
            cudaMemcpyAsync(b_dev, _rhs, sizeof(FloatingType) * size, cudaMemcpyHostToDevice, streams[0]);
        ...
        // TODO no longer needed when using only mpi
        ncclUniqueId id;
        ...
        {
            // TODO mpi broadcast of p_dev[0] in rank 0 (so copy it from device 0 into host) from rank 0 all other rank
            // TODO then copy in all other rank the vector from host into all devices
            ...
            // TODO substitute with mpi gather: we need to copy all the vectors Ap from device to its correspondent host
            // TODO then do a gather from all the rank to rank 0 and save the result in Ap0, in the end copy it to device 0 in rank 0
            ...
            // TODO no longer needed when using MPI
        ...
        // TODO: Finalizing NCCL - no more needed
        for (int i = 0; i < numDevices; i++) {
