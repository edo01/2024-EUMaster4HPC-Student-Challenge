...
int main(int argc, char **argv)
{
    ...
    printf("MPI Implementation, #Processes %d", num_procs);
    ...
    conjugate_gradients(matrix, rhs, sol, size, max_iters, rel_error, MPI_COMM_WORLD);
    printf("Process %d -> CG Done\n", rank);
    printf("\n");
    double *g_sol = nullptr;
    if (rank == 0)
    {
        g_sol = new double[size * num_procs];
    }
    MPI_Gather(sol, size, MPI_DOUBLE, g_sol, size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
